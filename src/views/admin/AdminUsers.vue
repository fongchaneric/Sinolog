...
<template>
  <div class="p-4 lg:p-6 max-w-4xl">
    <h1 class="text-xl font-bold text-gray-800 mb-4">Clients ({{ users.length }})</h1>

    <div class="card divide-y divide-gray-50">
      <div v-if="!users.length" class="p-8 text-center text-gray-400 text-sm">Aucun client inscrit pour le moment</div>
      <div v-for="u in users" :key="u.uid" class="flex items-center gap-3 px-4 py-3">
        ...
        <div class="flex-1 min-w-0">
          <p class="text-sm text-gray-800 truncate">{{ u.name || '(nom non renseigné)' }}</p>
