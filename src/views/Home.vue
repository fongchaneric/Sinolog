...
<template>
  <div class="max-w-7xl mx-auto">
    ...
    <div class="bg-orange-50 mx-2 mt-2 rounded-lg px-3 py-2.5 flex items-center justify-between text-xs text-brand-dark">
      <span class="flex items-center gap-1.5 font-medium">🛡️ La garantie Sinolog — des milliers d'entreprises, dans le monde entier</span>
      <span>›</span>
    </div>
    ...
    <section class="px-2 pt-3 pb-6">
      <h2 class="text-base font-bold text-gray-800 mb-2 px-1">Tendances du moment</h2>
      ...
      <div v-else-if="error" class="text-center text-sm text-gray-500 py-10">
        <p>{{ error }}</p>
        <p class="text-xs mt-1 text-gray-400">Vérifiez que CJ_API_KEY, CJ_API_EMAIL et CJ_API_BASE_URL sont bien configurés sur Vercel.</p>
      </div>

      <div v-else-if="!products.length" class="text-center text-sm text-gray-400 py-10">
        <p>Aucun produit trouvé pour le moment.</p>
        <p class="text-xs mt-1">Essayez un autre mot-clé ci-dessus, ou réessayez dans quelques instants.</p>
      </div>
