...
function friendlyError(e) {
  ...
  if (code.includes('invalid-credential') || code.includes('wrong-password') || code.includes('user-not-found')) {
    return 'Diso ny email na ny mot de passe'
  }
  if (code.includes('too-many-requests')) return 'Ezaho indray afaka kelikely'
  return e.message || 'Nisy olana'
}
</script>
  ...
  <div class="min-h-screen flex items-center justify-center bg-gradient-to-b from-brand-light/20 to-white px-4">
    <div class="w-full max-w-sm">
      ...
      <form @submit.prevent="submit" class="space-y-3">
        ...
        <button type="submit" class="btn-brand w-full !py-3" :disabled="loading">{{ loading ? '...' : 'Hiditra' }}</button>
      </form>

      <div class="flex items-center gap-3 my-5 text-xs text-gray-400">
        <div class="flex-1 h-px bg-gray-200" /> na <div class="flex-1 h-px bg-gray-200" />
      </div>
      ...
      <p class="text-center text-sm text-gray-500 mt-6">
        Mbola tsy manana kaonty? <router-link to="/register" class="text-brand font-semibold">Hisoratra anarana</router-link>
      </p>
    </div>
  </div>
</template>
