...
function friendlyError(e) {
  ...
  if (code.includes('too-many-requests')) return 'Réessayez dans quelques instants'
  ...
  <div class="min-h-screen flex items-center justify-center bg-gradient-to-b from-brand-light/20 to-white px-4">
    <div class="w-full max-w-sm">
      <div class="flex flex-col items-center mb-8">
        ...
        <h1 class="text-xl font-bold text-gray-800 mt-3">Bienvenue sur Sinolog</h1>
        <p class="text-xs text-gray-400 mt-1">Achetez des produits de Chine, facilement et en toute sécurité</p>
      </div>
      ...
      <form @submit.prevent="submit" class="space-y-3">
        ...
        <button type="submit" class="btn-brand w-full !py-3" :disabled="loading">{{ loading ? '...' : 'Se connecter' }}</button>
      </form>

      <div class="flex items-center gap-3 my-5 text-xs text-gray-400">
        <div class="flex-1 h-px bg-gray-200" /> ou <div class="flex-1 h-px bg-gray-200" />
      </div>
      ...
      <p class="text-center text-sm text-gray-500 mt-6">
        Vous n'avez pas de compte ? <router-link to="/register" class="text-brand font-semibold">S'inscrire</router-link>
      </p>
    </div>
  </div>
</template>
